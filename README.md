...
短編集に対応するため、「書籍」（物理的な一冊）と「作品」（書籍に収録された物語）を区別して扱う想定である。

- 書籍を登録すると、書籍タイトルを作品名とした作品が1つ自動作成される（例: 書籍「〇〇短編集」→ 作品「〇〇短編集」）
- 通常の小説は、自動作成された作品をそのまま使う
- 短編集は、自動作成された作品の作品名を実際の短編タイトルに変更し、同じ書籍に収録されている他の短編を作品として追加する
- 作品名は作品詳細画面に入らなくても編集できるようにする（例: 書籍詳細画面の作品一覧から直接編集）

情報の持たせ方の方針：

- **作品に紐づく情報**：登場人物・人物関係・出来事・気になった言葉/手がかり・メモ（短編ごとに内容が異なるため）
- **書籍に紐づく情報**：読書ステータス・現在のページ数・書籍全体のメモ・著者
- 著者は書籍に持たせる。アンソロジー（作品ごとに著者が違うケース）は現時点では主要ユースケースとして扱わず、必要になった時点で作品と著者の紐づけを検討する
- 書籍には必ず1作品以上が存在する（作品を0件にはできない）
- 通常の小説でも、DB 上は作品を1件作成するルールを維持する
- 初期実装では、作品が1件だけの書籍も「書籍一覧 → 書籍詳細（作品一覧を含む）→ 作品詳細」と同じ手順で遷移する。作品一覧を省略して直接作品詳細を開くなどの簡略化は、将来の改善として検討する
- 書籍タイトルは、書籍登録時に作品名の初期値としてコピーするだけで、その後は書籍タイトルと作品名を完全に独立して扱う（連動させない）
  - 書籍タイトルを変更しても作品名は変わらず、作品名を変更しても書籍タイトルは変わらない
  - 作品名を手動で変更したかどうかは DB で管理せず、両者が同じ名前かどうかによる自動追従もしない
  - 書籍タイトルの誤字修正などで作品名も直したい場合は、作品名を個別に修正する

> **未決定事項**：実装方法・DB 設計はまだ決まっていない。コンセプト冒頭の「一冊単位で一元管理する」という表現は、設計確定後に作品単位の管理に合わせて修正する予定。
