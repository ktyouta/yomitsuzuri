...
yomitsuzuri（よみつづり）は、小説を読みながら得た情報（登場人物・人物関係・出来事・気になった言葉や手がかり・自由なメモ）を記録・整理し、一冊の本の中身を自分なりに可視化できる読書支援アプリである。ミステリー作品で特に価値を発揮する想定だが、対象は小説・物語全般。将来的には読書履歴・ジャンル嗜好の管理や次に読む本の推薦機能への拡張も構想している。「小説を書くためのアプリ」ではなく、あくまで読書中の情報整理を中心とした読書支援アプリである。

情報登録の入力補助として AI 活用も構想している（音声入力→テキスト化→AI が人物・関係・メモ等に分類→ユーザーが確認・承認→承認分のみ DB 登録、という想定フロー。AI が確定情報を勝手に登録することはない）。詳細は README.md の「コンセプト」章を参照。

短編集に対応するため、「書籍」（物理的な一冊）と「作品」（収録された物語）を分けて扱う想定である。書籍を登録すると、書籍タイトルと同じ名前の作品が1つ自動で作成される（書籍タイトルは作品名の初期値としてコピーするだけで、以降は連動しない）。作品名は作品詳細画面に入る前（書籍詳細の作品一覧など）でも編集できるようにする。実装方法・DB 設計は未決定。詳細は README.md の「書籍と作品（短編集への対応）」を参照。

現在のコードベースは React + Hono + RPC を用いたフロントエンド・バックエンド統合基盤（認証付き）であり、上記の読書支援ドメイン機能（本・登場人物・関係・出来事・手がかり・メモ管理、AI入力支援）はまだ実装されていない。
